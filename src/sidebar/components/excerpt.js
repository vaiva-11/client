 ...
 * @prop {(a?: boolean) => any} [setCollapsed]
 ...
 * @prop {(a?: boolean) => any} [onCollapsibleChanged] - Called when the content height
 *   exceeds or falls below `collapsedHeight + overflowThreshold`.
 * @prop {(a?: boolean) => any} [onToggleCollapsed] - When `inlineControls` is `false`, this
...
function Excerpt({
  ...
  const contentElement = /** @type {Ref<HTMLDivElement>} */ (useRef());
